...
export default function DetailSection({
  ...
  const safeRelated = Array.isArray(comp?.relatedComponents)
  ...
  const handlePropChange = useCallback(
    (propName: string, value: string | boolean | number) => {
  ...
  const currentCode = useMemo(() => {
    ...
    Object.entries(propValues).forEach(([key, val]) => {
      ...
      if (typeof val === "boolean") {
        if (val) propStrings.push(key);
        else propStrings.push(`${key}={false}`);
      } else if (typeof val === "number") propStrings.push(`${key}={${val}}`);
      else if (val && val !== "" && val !== "default")
        propStrings.push(`${key}="${val}"`);
    });
    const propsStr = propStrings.length > 0 ? " " + propStrings.join(" ") : "";
    const childrenVal =
      typeof propValues["children"] === "string"
        ? propValues["children"]
        : comp.name;
  ...
  if (!comp) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-base text-[hsl(215,16%,47%)]">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-[hsl(215,16%,47%)]">
        <button
          onClick={onBack}
          className="flex items-center gap-1 hover:text-[hsl(222,47%,11%)] transition-colors"
        >
      ...
      <Card className="bg-[hsl(0,0%,98%)]/75 backdrop-blur-[16px] border border-white/[0.18] rounded-[0.875rem] shadow-md overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-[hsl(214,32%,91%)] bg-white/40">
          <span className="text-sm font-medium text-[hsl(215,16%,47%)]">
            Preview
          </span>
          <div className="flex gap-1">
            <button
              onClick={() => setCanvasBg("light")}
              className={`p-1.5 rounded ${canvasBg === "light" ? "bg-[hsl(214,32%,91%)]" : "hover:bg-white/60"}`}
            >
              <Sun className="h-3.5 w-3.5 text-[hsl(215,16%,47%)]" />
            </button>
            <button
              onClick={() => setCanvasBg("dark")}
              className={`p-1.5 rounded ${canvasBg === "dark" ? "bg-[hsl(214,32%,91%)]" : "hover:bg-white/60"}`}
            >
              <Moon className="h-3.5 w-3.5 text-[hsl(215,16%,47%)]" />
            </button>
            <button
              onClick={() => setCanvasBg("grid")}
              className={`p-1.5 rounded ${canvasBg === "grid" ? "bg-[hsl(214,32%,91%)]" : "hover:bg-white/60"}`}
            >
              <Grip className="h-3.5 w-3.5 text-[hsl(215,16%,47%)]" />
            </button>
          </div>
        </div>
        <div
          className={`${canvasBgClass} p-8 min-h-[200px] flex items-center justify-center transition-colors duration-200`}
        >
          <ComponentPreview
            name={comp.name}
            propValues={propValues}
            variant={safeVariants[selectedVariant] ?? "default"}
            isDark={canvasBg === "dark"}
          />
        </div>
      </Card>
              ...
              {safeProps.length === 0 ? (
                ...
                safeProps.map((prop) => (
                  <div key={prop.name} className="flex items-center gap-4">
                    <label className="text-sm font-medium text-[hsl(222,47%,11%)] w-28 shrink-0">
                      {prop.name}
                      {prop.required && (
                    ...
                    {prop.type === "boolean" ? (
                      <Switch
                        ...
                        onCheckedChange={(v) => handlePropChange(prop.name, v)}
                      />
                    ) : prop.type.includes("|") ? (
                      <Select
                      ...
                      <Input
                        ...
                        className="h-8 text-sm flex-1 bg-white/60 border-[hsl(214,32%,91%)] rounded-lg"
                      />
                    ) : (
                      <Input
                        value={String(propValues[prop.name] ?? "")}
        ...
        <TabsContent value="code" className="mt-4">
          <Card className="bg-[hsl(0,0%,98%)]/75 backdrop-blur-[16px] border border-white/[0.18] rounded-[0.875rem] shadow-md overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b border-[hsl(214,32%,91%)] bg-white/40">
              ...
              <button
                onClick={handleCopy}
                className="flex items-center gap-1 text-sm text-[hsl(215,16%,47%)] hover:text-[hsl(222,47%,11%)] transition-colors"
              >
                {copied ? (
                  <>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-sm font-semibold w-16">
                      Required
                    </TableHead>
                    <TableHead className="text-sm font-semibold">
                      Description
                ...
                <TableBody>
                  {safeProps.map((prop) => (
                    <TableRow key={prop.name}>
                      <TableCell className="text-sm font-mono text-indigo-600">
                        {prop.name}
                      </TableCell>
                      <TableCell className="text-sm font-mono text-[hsl(215,16%,47%)]">
                        {prop.type}
                      </TableCell>
                      ...
                      <TableCell className="text-sm">
                        {prop.required ? (
                          <Badge
                            variant="secondary"
                            className="text-[9px] bg-red-50 text-red-600"
                          >
                            Yes
                          </Badge>
                        ) : (
                          <span className="text-[hsl(215,16%,47%)]">No</span>
                        )}
                      </TableCell>
                      <TableCell className="text-sm text-[hsl(215,16%,47%)] leading-relaxed">
                        {prop.description}
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <TabsContent value="guidelines" className="mt-4">
          <Card className="bg-[hsl(0,0%,98%)]/75 backdrop-blur-[16px] border border-white/[0.18] rounded-[0.875rem] shadow-md">
            <CardContent className="pt-5 pb-4 px-5 space-y-5">
              <div>
                <h3 className="text-base font-semibold text-[hsl(222,47%,11%)] mb-2">
                  Usage Guidelines
                </h3>
                <div className="text-sm text-[hsl(215,16%,47%)] leading-relaxed whitespace-pre-line">
                  {comp.guidelines || "No guidelines available."}
                </div>
              </div>
              <Separator />
              <div>
                ...
                <div className="text-sm text-[hsl(215,16%,47%)] leading-relaxed whitespace-pre-line">
                  {comp.accessibility || "No accessibility notes available."}
                </div>
              </div>
              {safeRelated.length > 0 && (
                <>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {safeRelated.map((r) => {
                        const found = Array.isArray(allComponents)
                          ? allComponents.find((c) => c.name === r)
                          : null;
                        return (
                          <button
                            key={r}
                            onClick={() =>
                              found
                                ? onNavigateToComponent(found.id)
                                : undefined
                            }
...
function ComponentPreview({
  ...
  const children =
    typeof propValues["children"] === "string" && propValues["children"]
      ? propValues["children"]
      : name;
  ...
  switch (name) {
    case "Button": {
      ...
      return (
        <button
          ...
          disabled={disabled}
        >
          {propValues["loading"] ? "Loading..." : children}
        </button>
      );
    }
    case "Input": {
      ...
      return (
        <input
          type={type}
          placeholder={String(propValues["placeholder"] || "Enter text...")}
          disabled={disabled}
          className={`${sizeClasses} rounded-lg border ${borderColor} bg-transparent ${textColor} outline-none focus:ring-2 focus:ring-indigo-500 w-64 ${disabled ? "opacity-50" : ""}`}
          readOnly
        />
      );
    }
    case "Badge": {
      ...
      return (
        <span
          className={`px-2.5 py-0.5 rounded-full text-sm font-medium ${colors[variant] || colors.default}`}
        >
          {children}
        </span>
      );
    ...
    case "Avatar": {
      ...
      return (
        <div
          className={`${sizeMap[size] || sizeMap.md} rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-semibold`}
        >
          {String(propValues["initials"] || "AZ")}
        </div>
      );
    }
    case "Toggle": {
    ...
    case "Tooltip": {
      return (
        <div className="relative inline-block">
          <div
            className={`absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-lg bg-slate-800 text-white text-sm whitespace-nowrap shadow-lg`}
          >
          ...
          <button className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-sm">
            Hover me
          </button>
        </div>
      );
    ...
    case "DataTable": {
      return (
        <div
          className={`border ${borderColor} rounded-lg overflow-hidden w-80`}
        >
          <div className="grid grid-cols-3 gap-0">
            {["Name", "Role", "Status"].map((h) => (
              <div
                key={h}
                className={`px-3 py-2 text-sm font-semibold ${isDark ? "bg-slate-800 text-slate-300" : "bg-slate-50 text-slate-600"} border-b ${borderColor}`}
              >
                {h}
              </div>
            ))}
            {[
              ...
              row.map((cell, j) => (
                <div
                  key={`${i}-${j}`}
                  className={`px-3 py-2 text-sm ${textColor} border-b ${borderColor}`}
                >
                  {cell}
                </div>
              ))
    ...
    case "Select": {
      return (
        <div
          className={`px-4 py-2 rounded-lg border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} ${textColor} text-base w-48 flex items-center justify-between`}
        >
          <span>{String(propValues["placeholder"] || "Select option...")}</span>
    ...
    case "DatePicker": {
      return (
        <div
          className={`px-4 py-2 rounded-lg border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} ${textColor} text-base w-48 flex items-center justify-between`}
        >
          <span className={mutedColor}>Pick a date...</span>
          <span className={mutedColor}>cal</span>
        </div>
      );
    }
    case "Combobox": {
      return (
        <div
          className={`px-4 py-2 rounded-lg border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} ${textColor} text-base w-48`}
        >
          <span className={mutedColor}>Search...</span>
        </div>
      );
    }
    case "Card": {
      return (
        <div
          className={`rounded-xl border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} shadow-md w-64 overflow-hidden`}
        >
          <div className="px-4 py-3 border-b border-inherit">
            <p className={`text-base font-semibold ${textColor}`}>
              {String(propValues["title"] || "Card Title")}
            </p>
          </div>
    ...
    case "Dialog": {
      return (
        <div
          className={`rounded-xl border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} shadow-xl w-72 overflow-hidden`}
        >
          <div className="px-4 py-3 border-b border-inherit flex items-center justify-between">
            <p className={`text-base font-semibold ${textColor}`}>
              {String(propValues["title"] || "Dialog Title")}
            </p>
          ...
          <div className="px-4 py-3 border-t border-inherit flex justify-end gap-2">
            <button className="px-3 py-1 text-sm rounded bg-slate-100 text-slate-700">
              Cancel
            </button>
        ...
        <div className="w-64">
          <div className={`flex border-b ${borderColor} gap-0`}>
            {["Tab 1", "Tab 2", "Tab 3"].map((t, i) => (
              <div
                key={t}
                className={`px-3 py-2 text-sm font-medium ${i === 0 ? `border-b-2 border-indigo-600 text-indigo-600` : mutedColor}`}
              >
                {t}
              </div>
            ))}
          ...
          <div className="py-3">
            <p className={`text-sm ${mutedColor}`}>Tab content area</p>
          </div>
        </div>
      );
    }
    case "UserProfile": {
      return (
        <div
          className={`rounded-xl border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} shadow-md w-64 p-4`}
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-semibold text-base">
              JD
            </div>
            <div>
              <p className={`text-base font-semibold ${textColor}`}>
                {String(propValues["name"] || "Jane Doe")}
              </p>
              <p className={`text-sm ${mutedColor}`}>
                {String(propValues["role"] || "Product Designer")}
              </p>
            </div>
          </div>
        </div>
    ...
    case "PricingCard": {
      return (
        <div
          className={`rounded-xl border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} shadow-md w-56 p-5`}
        >
          <p
            className={`text-sm font-semibold ${mutedColor} uppercase tracking-wide`}
          >
            {String(propValues["tier"] || "Pro")}
          </p>
          <p className={`text-2xl font-bold ${textColor} mt-1`}>
            ${String(propValues["price"] || "29")}
            <span className={`text-sm ${mutedColor} font-normal`}>/mo</span>
          </p>
          <div className="mt-3 space-y-1.5">
            {["Feature A", "Feature B", "Feature C"].map((f) => (
              <p key={f} className={`text-sm ${mutedColor}`}>
                {f}
              </p>
            ))}
          </div>
          <button className="mt-4 w-full py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium">
            Get Started
          </button>
        </div>
      );
    ...
    case "StatCard": {
      return (
        <div
          className={`rounded-xl border ${borderColor} ${isDark ? "bg-slate-800" : "bg-white"} shadow-md w-48 p-4`}
        >
          <p className={`text-sm ${mutedColor} font-medium`}>
            {String(propValues["label"] || "Revenue")}
          </p>
          <p className={`text-xl font-bold ${textColor} mt-1`}>
            {String(propValues["value"] || "$12,345")}
          </p>
          <p className="text-sm text-emerald-600 mt-1">
            {String(propValues["change"] || "+12.5%")}
          </p>
        </div>
      );
    }
    default:
      return (
        <div
          className={`px-6 py-4 rounded-lg border ${borderColor} ${textColor} text-base`}
        >
          {name} Preview
        </div>
      );
